    ...
    class LayoutHook < Redmine::Hook::ViewListener
      ...
      def view_issues_show_details_bottom(context={ })
        begin
          ...
          snippet = '<div class="splitcontent"><div class="splitcontentleft">'

          project = context[:project]

          if issue.is_story?
            snippet += "<div class='story-points attribute'><div class='label'><span>#{l(:field_story_points)}</span>:</div>"
            snippet += "<div class='value'>#{RbStory.find(issue.id).points_display}</div></div>"
            unless issue.remaining_hours.nil?
              snippet += "<div class='remaining-hours attribute'><div class='label'><span>#{l(:field_remaining_hours)}</span>:</div>"
              snippet += "<div class='value'>#{l_hours(issue.remaining_hours)}</div></div>"
            end
            vbe = issue.velocity_based_estimate
            snippet += "<div class='velocity attribute'><div class='label'><span>#{l(:field_velocity_based_estimate)}</span>:</div>"
            snippet += "<div class='value'>#{vbe ? vbe.to_s + ' days' : '-'}</div></div></div><div class='splitcontentleft'>"
            ...
            unless issue.release_id.nil?
              ...
              snippet += "<div class='value'>#{relation_translate}</div></div>"
            end
          end

          if issue.is_task? && User.current.allowed_to?(:update_remaining_hours, project) != nil
            snippet += "<div class='remaining-hours attribute'><div class='label'><span>#{l(:field_remaining_hours)}</span>:</div>"
            snippet += "<div class='value'>#{issue.remaining_hours}</div></div>"
          end
          snippet += "</div></div>"
